...
class LogStatusOrder extends StatefulWidget {
  const LogStatusOrder({Key? key, required this.noNota}) : super(key: key);

  final String noNota;

  @override
  State<LogStatusOrder> createState() => _LogStatusOrderState();
}

class _LogStatusOrderState extends State<LogStatusOrder> {
  ...
  Future<List<Map<String, dynamic>>> fetchLogs() async {
    ...
    final responseLogs = await http.get(Uri.parse(Config.baseUrlApi +
        'app-api/order/log/?key=0cc7da679bea04fea453c8062e06514d&iduser=' +
        (await auth.idUser) +
        '&sess_id=' +
        (await auth.sessId) +
    ...
    if (responseLogs.statusCode == 200) {
      ...
      print('Failed to load order log');
  ...
  Future<Map<String, dynamic>> kirimLog() async {
    ...
    var res = await http.post(
      Uri.parse(Config.baseUrlApi + 'app-api/order/kirim-log/'),
      body: bodyPost,
    );
    print(res.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Log Status Order'),
      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding: const EdgeInsets.symmetric(
                                      horizontal: 4.0),
                                  child: Html(data: logs[index]['user_pesan']),
                                ),
                              ],
                            ),
                          ),
